<script>
  ...
  onMount(async () => {
    try {
      const res = await fetch("http://3.82.98.48:1337/api/home", {
        headers: {
...
<!-- Body -->
<div
  class="h-screen w-screen flex flex-col items-center justify-start bg-black text-white"
>
  <div class="max-w-[1440px] py-6">
    ...
    >
      ...
      <div class="w-[60rem] flex flex-row items-center justify-center gap-6">
        <button
          class="rounded-lg border-white bg-white text-black border px-4 py-2"
